...
public abstract class PlayerEntityMixin {
    ...
    public void longSwordSweeping(Entity target, CallbackInfo ci) {
        ...
        if (target.isAttackable()) {
            ...
            if (weapon instanceof LongSwordItem) {
            ...
            // Machete
            if (weapon instanceof MacheteItem machete && target instanceof PlayerEntity targetPlayer
                    && targetPlayer.getBlockingItem() != null && player.getAttackCooldownProgress(0.5F) > 0.65F
                    && machete.disableChance + (crit(targetPlayer) ? Math.ceil(Math.sqrt((double) machete.disableChance
                    / 10)) : 0) >= Random.create().nextInt(100) + 1) {

                targetPlayer.getItemCooldownManager().set(targetPlayer.getBlockingItem(), 100);
...
public void saiAttackPrevention(Entity target, CallbackInfo ci) {
    ...
    PlayerEntity player = (PlayerEntity) (Object) this;
    Item weapon = player.getMainHandStack().getItem();
    Set<Item> cooldownWeapons = SaiItem.cooldownFromSai.get(player.getUuid());
        ...
        if (amount >= 25 && 4 * ((amount >= 50 ? 50 : amount) - 25) > Random.create().nextInt(101)) {
            ...
            if (weapon.disarmChance >= Random.create().nextInt(100) + 1) {
                ...
                if (attacker instanceof PlayerEntity attackingPlayer && !attackingPlayer.isCreative()) {
                    ...
                    disableItem(attackingPlayer, Items.SHIELD, (int) (weapon.disarmDuration / 1.5));

                    ItemStack offHandStack = attackingPlayer.getOffHandStack();

                    disableItem(attackingPlayer, offHandStack.getItem(), weapon.disarmDuration);

                    for (int i = 0; i < 9; i++) {
                        ItemStack stack = attackingPlayer.getInventory().getStack(i);

                        disableItem(attackingPlayer, stack.getItem(), weapon.disarmDuration);
                    }
...
@Unique
public void disableSais(PlayerEntity player, int duration) {
    for (Item sai : sais) {
        player.getItemCooldownManager().set(sai.getDefaultStack(), duration);
    }
}
...
@Unique
public void disableItem(PlayerEntity player, Item weapon, int disarmDuration) {

    ItemStack stack = weapon.getDefaultStack();

    if (!stack.isEmpty() && !(player.getItemCooldownManager().isCoolingDown(stack))) {
        player.getItemCooldownManager().set(stack, disarmDuration);
        addItemToCooldown(player, weapon);

        SaiCooldownScheduler.schedule(disarmDuration, () -> {
            removeWeaponFromCooldown(player, weapon);
        });
    }
}
...
public void addItemToCooldown(PlayerEntity player, Item weapon) { // The player can't attack whilst holding these items
    ...
    SaiItem.cooldownFromSai.computeIfAbsent(player.getUuid(), u -> new HashSet<>()).add(weapon);
    if (!((PlayerEntity) (Object) this).getWorld().isClient()) sendPacket(player);
}

@Unique
public void removeWeaponFromCooldown(PlayerEntity player, Item item) {

    Set<Item> weapons = SaiItem.cooldownFromSai.get(player.getUuid());

    if (weapons != null) {
        weapons.remove(item);

        if (weapons.isEmpty()) {
            SaiItem.cooldownFromSai.remove(player.getUuid());
        }
...
@Unique
public void sendPacket(PlayerEntity player) {

    if (!player.getWorld().isClient() && player instanceof ServerPlayerEntity serverPlayer) {
        ServerPlayNetworking.send(serverPlayer, new CooldownPacket(SaiItem.cooldownFromSai));
    }
}
